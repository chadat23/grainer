...
import { TempGeneratorFactory } from '@/services/tempGenerator/TempGeneratorFactory';
...
export default function Home() {
  ...
  const [lineTemps, setLineTemps] = useState<Map<number, number>>(new Map());
  ...
  const handleSave = () => {
    if (!sourceFile) {
      console.log('No source file to save');
      return;
    }
    ...
    reader.onload = (e) => {
      ...
      link.href = url;
      link.download = sourceFile.name.replace('.gcode', '_modified.gcode');
      ...
      console.log('G-code file saved with temperature commands:', {
        originalLines: lines.length,
        temperatureCommands: lineTemps.size,
        sourceFileName: sourceFile.name,
        outputFileName: sourceFile.name.replace('.gcode', '_modified.gcode')
      });
    };
  ...
  // Calculate temperatures when relevant parameters change
  useEffect(() => {
    if (commands.length > 0) {
      const tempGenerator = TempGeneratorFactory.createTempGenerator('layer');
      const { lineTemps: newLineTemps } = tempGenerator.calculateTemps({
        commands,
      ...
      setLineTemps(newLineTemps);
    }
  }, [commands, minTempText, nominalDarkTempText, maxDarkTempText, darkTempDeviation, 
     lightNominalWidth, lightWidthStandardDeviation, darkNominalWidth, darkWidthStandardDeviation,
     transitionNominalWidth, transitionStandardDeviation, seed]);
  ...
  console.log('Home component rendering - END');
}
